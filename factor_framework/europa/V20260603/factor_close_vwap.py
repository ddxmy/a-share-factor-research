...
# ---- 因子元信息 ----
FACTOR_NAME = 'close_vwap'
FACTOR_TYPE = 'T-1_factor'


def factor_close_vwap(start_date, end_date, md_data_file, return_fillna_dic=False):
    """基于收盘价与 VWAP 的 alpha 因子（T-1 日频）。

    计算方法：因子值 = (close - vwap) / vwap。
    ...
    Returns:
        pd.DataFrame: 包含因子列 'close_vwap' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    if 'close' not in md_data.columns or 'vwap' not in md_data.columns:
        raise RuntimeError('market data must contain close and vwap columns')

    close_df = md_data['close'].unstack()
    vwap_df = md_data['vwap'].unstack()
    alpha_df = ((close_df - vwap_df) / (vwap_df+0.000001))
    factor_series = alpha_df.stack()
...
if __name__ == '__main__':
    ...
    factor_df = factor_close_vwap(start_date, end_date, md_data_file, return_fillna_dic=False)
