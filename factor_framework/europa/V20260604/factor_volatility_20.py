...
def factor_volatility_20(start_date, end_date, md_data_file, return_fillna_dic=False):
    """20日滚动波动率因子（T-1 日频）。

    计算方法：先计算每只股票的日收益率，再基于过去20个交易日的收益率
    计算滚动标准差并年化，最终因子值为 annualized volatility。
    ...
    Returns:
        pd.DataFrame: 包含因子列 'volatility_20' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    close_df = md_data['pct_chg'].unstack()/100
    vol_df = close_df.rolling(20, min_periods=1).std()
    factor_series = vol_df.stack()
...
if __name__ == '__main__':
    ...
    factor_df = factor_volatility_20(start_date, end_date, md_data_file, return_fillna_dic=False)
