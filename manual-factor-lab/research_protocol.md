...
This laboratory is a frozen evaluation plan for the ten factor IDs in
`factor_registry.csv`. It contains no market data, calculated signals, or run
artifacts. Each formula is implemented as an independently importable module
and is tested against its field and timing contract before evaluation.

## Data and timing contract

The eligible universe is point-in-time CSI 300 membership on every trading-day
signal date. Required fields and exact definitions are frozen in the registry. Daily
...
The outcome is each security's next 1D, 5D, or 20D open-to-open return less
the contemporaneous CSI 300 open-to-open return. Signal values are
...
For every factor separately, evaluate daily cross-sectional Spearman Rank IC
against each outcome horizon over the five locked out-of-sample test years
2021--2025. Report the equal-day mean Rank IC, standard deviation, ICIR,
positive-day ratio, annual summaries, and cumulative Rank IC. Estimate the
mean Rank IC with a Bartlett Newey--West HAC standard error; the lag equals
the label overlap for each horizon. The sign test is one-sided in the
pre-registered direction from the registry; all other diagnostics are
descriptive.
...
2. It has at least 500 eligible daily out-of-sample observations across the
   locked 2021--2025 test period.
3. Its mean 1D Rank IC has the pre-registered direction and a one-sided
