...
# ---- 因子元信息 ----
FACTOR_NAME = 'xzt_20260622_4'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_Rank(SignedPower(Div(Sub(close, vwap), vwap), 3))'

def factor_xzt_20260622_4(start_date, end_date, md_data_file, return_fillna_dic=False):
    """对VWAP偏离做三次方放大（保留符号），再做截面排名
    ...
    Returns:
        pd.DataFrame: 包含因子列 'xzt_20260622_4' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # SignedPower(..., 3): 三次方放大（保留符号）
    signed_power = np.sign(vwap_dev_pct) * np.abs(vwap_dev_pct) ** 3

    # Cs_Rank(...): 截面排名
    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = signed_power.unstack().rank(axis=1, pct=True).stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_xzt_20260622_4(
