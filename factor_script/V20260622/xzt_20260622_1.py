...
FORMULA = 'Cs_ZScore(Ts_Return(close, 60))'

def factor_xzt_20260622_1(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去60日收益率的截面标准化值
    ...
    Returns:
        pd.DataFrame: 包含因子列 'xzt_20260622_1' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # ---- 因子计算 ----
    # Ts_Return(close, 60): 60日累计收益率
    close_panel = md_data['close'].unstack()
    returns_60 = close_panel.pct_change(60)

    # Cs_ZScore(...): 截面标准化
    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = ((returns_60 - returns_60.mean(axis=1).values[:, np.newaxis]) /
                               (returns_60.std(axis=1).values[:, np.newaxis] + 1e-12)).stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_xzt_20260622_1(
