...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260701_12'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Mul(Ts_Return(close, 1), Sqrt(Ts_DecayExp(Volatility_Ratio(5, 20), 10)))'

def factor_jyb_20260701_12(start_date, end_date, md_data_file, return_fillna_dic=False):
    """价格变动 × 10日指数衰减加权的5/20波动率比率的平方根
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_12' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # 5/20波动率比率 → 10日衰减加权 → 平方根
    vol_ratio = close_panel.rolling(5, min_periods=1).std() / (close_panel.rolling(20, min_periods=1).std() + 1e-12)
    vol_smoothed_sqrt = np.sqrt(vol_ratio.ewm(alpha=0.1, min_periods=1).mean())

    # Mul(...): 收益率 × 波动率
    factor_val = ret_1d * vol_smoothed_sqrt
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_12(
