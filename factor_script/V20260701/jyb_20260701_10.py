...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260701_10'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Neg(Ts_ZScore(Ts_Residual(close, vwap, 20), 10))'

def factor_jyb_20260701_10(start_date, end_date, md_data_file, return_fillna_dic=False):
    """负的20日收盘价对VWAP残差的10日ZScore
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_10' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # Ts_Residual(close, vwap, 20): close~vwap 的20日滚动回归残差
    x_mean = vwap_panel.rolling(20, min_periods=1).mean()
    y_mean = close_panel.rolling(20, min_periods=1).mean()
    x_var = ((vwap_panel - x_mean) ** 2).rolling(20, min_periods=1).mean()
    xy_cov = ((vwap_panel - x_mean) * (close_panel - y_mean)).rolling(20, min_periods=1).mean()
    beta = xy_cov / (x_var + 1e-12)
    residual = close_panel - (y_mean + beta * (vwap_panel - x_mean))

    # Ts_ZScore(..., 10): 残差的10日ZScore
    res_mean = residual.rolling(10, min_periods=1).mean()
    res_std = residual.rolling(10, min_periods=1).std()
    zscore = (residual - res_mean) / (res_std + 1e-12)
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_10(
