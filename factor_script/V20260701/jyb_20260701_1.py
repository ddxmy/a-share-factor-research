...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260701_1'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Add(Sign(Sub(close, BBands_Upper(close, 20, 2))), Ts_Rank(Max(Sub(close, BBands_Lower(close, 20, 2)), 0), 10))'

def factor_jyb_20260701_1(start_date, end_date, md_data_file, return_fillna_dic=False):
    """收盘价相对布林带上轨的突破方向符号 + 收盘价相对布林带下轨正偏离的10日时序排名
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_1' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # Sign(Sub(close, BBands_Upper)): 相对上轨的突破方向
    sign_part = np.sign(close_panel - bb_upper)

    # Ts_Rank(Max(Sub(close, BBands_Lower), 0), 10): 相对下轨正偏离的10日时序排名
    above_lower = (close_panel - bb_lower).clip(lower=0)
    ts_rank_part = above_lower.rolling(10, min_periods=1).apply(
        lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)

    # Add(...): 两部分相加
    factor_val = sign_part + ts_rank_part
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_1(
