...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260701_6'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Ts_DecayExp(Div(vwap, Sub(high, low)), 10)'

def factor_jyb_20260701_6(start_date, end_date, md_data_file, return_fillna_dic=False):
    """10日指数衰减加权的均价与日振幅（high-low）之比（价格效率指标）
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_6' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # vwap / 日振幅
    price_efficiency = vwap_panel / (high_panel - low_panel + 1e-12)

    # Ts_DecayExp(..., 10): 10日指数衰减加权
    decayed = price_efficiency.ewm(alpha=0.1, min_periods=1).mean()

    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = decayed.stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_6(
