...
# ---- 因子元信息 ----
FACTOR_NAME = 'xzt_20260701_5'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_Rank(Div(Inverse(Ts_Quantile(turn, 20, 0.95)), Ts_Mean(turn, 20)))'

def factor_xzt_20260701_5(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去20日换手率95分位数的倒数除以过去20日换手率均值的截面排名
    ...
    # Ts_Quantile(turn, 20, 0.95): 过去20日换手率95分位数
    quantile_95 = md_data['turn'].unstack().rolling(20, min_periods=1).quantile(0.95)
    # Ts_Mean(turn, 20): 过去20日换手率均值
    mean_20 = md_data['turn'].unstack().rolling(20, min_periods=1).mean()
    # Div(Inverse(quantile_95), mean_20)
    result = (1.0 / quantile_95) / mean_20
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_xzt_20260701_5(
