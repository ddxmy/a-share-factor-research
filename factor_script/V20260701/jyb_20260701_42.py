...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260701_42'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_Percentile(Ts_WMA(SignedPower(Trend_Strength(turn, 15), 0.5), 5))'

def factor_jyb_20260701_42(start_date, end_date, md_data_file, return_fillna_dic=False):
    """5日WMA加权的15日换手率趋势强度0.5次方带符号幂的截面百分位
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_42' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # 15日趋势强度: 线性回归斜率的绝对值
    trend_x = np.arange(15)
    trend_slope_abs = turn_panel.rolling(15, min_periods=1).apply(
        lambda y: np.polyfit(trend_x[:len(y)], y, 1)[0] if len(y) > 1 else 0, raw=True
    ).abs()

    # SignedPower(..., 0.5): 带符号平方根
    signed_sqrt = np.sign(trend_slope_abs) * np.abs(trend_slope_abs) ** 0.5

    # Ts_WMA(..., 5): 5日WMA
    wma_weights = np.arange(1, 6, dtype=float)
    wma_weights = wma_weights / wma_weights.sum()
    wma_5 = signed_sqrt.rolling(5, min_periods=1).apply(lambda x: np.dot(x, wma_weights), raw=True)

    # Cs_Percentile(...): 截面百分位
    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = wma_5.rank(axis=1, pct=True).stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_42(
