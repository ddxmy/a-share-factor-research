...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260701_21'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_ZScore(Ts_DecayExp(Abs(Sub(Div(close, open), 1)), 10))'

def factor_jyb_20260701_21(start_date, end_date, md_data_file, return_fillna_dic=False):
    """10日指数衰减加权的日收益率绝对值（|close/open - 1|）的截面ZScore
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_21' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    open_panel = md_data['open'].unstack()

    # |close/open - 1| → 10日衰减加权
    intraday_ret_abs = (close_panel / (open_panel + 1e-12) - 1).abs()
    decayed = intraday_ret_abs.ewm(alpha=0.1, min_periods=1).mean()
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_21(
