...
def factor_jyb_20260703_4(start_date, end_date, md_data_file, return_fillna_dic=False):
    """收益与换手20日协动性的10日衰减加权截面排名（价量联动）

    复现公式: Cs_Rank(Ts_DecayExp(Ts_Correlation(Ts_Return(close, 1), turn, 20), 10))
    原始分数: 51.7
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_4' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Ts_DecayExp(Ts_Correlation(Ts_Return(close, 1), turn, 20), 10))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_4(
