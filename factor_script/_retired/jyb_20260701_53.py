...
def factor_jyb_20260701_53(start_date, end_date, md_data_file, return_fillna_dic=False):
    """10日线性衰减加权的以开盘价为基准的日内振幅率的截面5%-95%缩尾

    复现公式: Cs_Winsorize(Ts_DecayLinear(Div(Sub(high, low), open), 10), 0.05, 0.95)
    原始分数: 50.06
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_53' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Winsorize(Ts_DecayLinear(Div(Sub(high, low), open), 10), 0.05, 0.95)")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_53(
