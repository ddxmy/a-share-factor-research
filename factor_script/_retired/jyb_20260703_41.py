...
def factor_jyb_20260703_41(start_date, end_date, md_data_file, return_fillna_dic=False):
    """高波动时取收益90分位/成交量10分位比截面排名，否则取反向收益排名

    复现公式: If(Greater(Volatility_Ratio(5, 20), 1.5), Cs_Rank(Div(Ts_Quantile(Ts_Return(close, 1), 20, 0.9), Ts_Quantile(volume, 10, 0.1))), Neg(Cs_Rank(Ts_Return(close, 1))))
    原始分数: 37.7
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_41' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("If(Greater(Volatility_Ratio(5, 20), 1.5), Cs_Rank(Div(Ts_Quantile(Ts_Return(close, 1), 20, 0.9), Ts_Quantile(volume, 10, 0.1))), Neg(Cs_Rank(Ts_Return(close, 1))))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_41(
