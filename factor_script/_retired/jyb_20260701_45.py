...
def factor_jyb_20260701_45(start_date, end_date, md_data_file, return_fillna_dic=False):
    """5日线性衰减加权的10日MAD的20日收益率对换手率残差的截面排名

    复现公式: Cs_Rank(Ts_DecayLinear(Ts_MAD(Ts_Residual(Ts_Return(close, 1), turn, 20), 10), 5))
    原始分数: 55.5
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_45' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Ts_DecayLinear(Ts_MAD(Ts_Residual(Ts_Return(close, 1), turn, 20), 10), 5))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_45(
