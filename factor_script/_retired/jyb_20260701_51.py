...
def factor_jyb_20260701_51(start_date, end_date, md_data_file, return_fillna_dic=False):
    """12日指数衰减加权的20日涨跌幅对对数成交额残差绝对值的截面ZScore

    复现公式: Cs_ZScore(Ts_DecayExp(Abs(Ts_Residual(Ts_Return(close, 20), Log(amt), 20)), 12))
    原始分数: 57.17
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_51' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_ZScore(Ts_DecayExp(Abs(Ts_Residual(Ts_Return(close, 20), Log(amt), 20)), 12))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_51(
