...
# ---- 因子元信息 ----
FACTOR_NAME = 'Turnover_Stability_Adjusted_Mean_Rank'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Turnover_Stability_Adjusted_Mean_Rank(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去20日换手率标准差的倒数除以过去20日换手率均值加0.01的截面排名

    复现公式: Cs_Rank(Div(Inverse(Ts_Std(turn, 20)), Add(Ts_Mean(turn, 20), 0.01)))
    复现分数: 58.78
    """
    ...
    # Div(Inverse(std_20), Add(mean_20, 0.01))
    result = (1.0 / std_20) / (mean_20 + 0.01)
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_Turnover_Stability_Adjusted_Mean_Rank(
