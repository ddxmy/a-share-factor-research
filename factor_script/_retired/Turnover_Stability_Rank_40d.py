...
# ---- 因子元信息 ----
FACTOR_NAME = 'Turnover_Stability_Rank_40d'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Turnover_Stability_Rank_40d(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去40日换手率标准差的倒数除以过去40日换手率均值的截面排名

    复现公式: Cs_Rank(Div(Inverse(Ts_Std(turn, 40)), Ts_Mean(turn, 40)))
    复现分数: 61.78
    """
    ...
    start_date_ = int(get_tradingday(None, start_date=20120101,
                      end_date=start_date, shift=-50)[0])
    ...
    # Ts_Std(turn, 40): 过去40日换手率标准差
    std_40 = md_data['turn'].unstack().rolling(40, min_periods=1).std()
    # Ts_Mean(turn, 40): 过去40日换手率均值
    mean_40 = md_data['turn'].unstack().rolling(40, min_periods=1).mean()
    # Div(Inverse(std_40), mean_40)
    result = (1.0 / std_40) / mean_40
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_Turnover_Stability_Rank_40d(
