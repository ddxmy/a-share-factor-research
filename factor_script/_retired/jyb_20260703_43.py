...
def factor_jyb_20260703_43(start_date, end_date, md_data_file, return_fillna_dic=False):
    """波动率比>1.5时取收盘-均价绝对值排名，否则取换手率波动倒数排名

    复现公式: If(Greater(Volatility_Ratio(5, 20), 1.5), Cs_Rank(Abs(Sub(close, vwap))), Cs_Rank(Inverse(Ts_Std(turn, 20))))
    原始分数: 56.8
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_43' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("If(Greater(Volatility_Ratio(5, 20), 1.5), Cs_Rank(Abs(Sub(close, vwap))), Cs_Rank(Inverse(Ts_Std(turn, 20))))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_43(
