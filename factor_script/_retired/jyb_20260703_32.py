...
def factor_jyb_20260703_32(start_date, end_date, md_data_file, return_fillna_dic=False):
    """RSI>60且高波动时取收盘-均价符号平方根，否则取反向换手排名

    复现公式: If(And(Greater(RSI(close, 14), 60), Greater(Volatility_Ratio(5, 20), 1.2)), Sqrt(Sign(Sub(close, vwap))), Neg(Cs_Rank(turn)))
    原始分数: 44.3
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_32' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("If(And(Greater(RSI(close, 14), 60), Greater(Volatility_Ratio(5, 20), 1.2)), Sqrt(Sign(Sub(close, vwap))), Neg(Cs_Rank(turn)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_32(
