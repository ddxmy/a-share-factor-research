...
def factor_jyb_20260703_42(start_date, end_date, md_data_file, return_fillna_dic=False):
    """RSI超买(>70)且正偏时取反向收益排名，否则取收益排名（超买反转）

    复现公式: If(And(Greater(RSI(close, 14), 70), Greater(Ts_Skewness(Ts_Return(close, 1), 20), 0)), Neg(Cs_Rank(Ts_Return(close, 1))), Cs_Rank(Ts_Return(close, 1)))
    原始分数: 35.8
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_42' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("If(And(Greater(RSI(close, 14), 70), Greater(Ts_Skewness(Ts_Return(close, 1), 20), 0)), Neg(Cs_Rank(Ts_Return(close, 1))), Cs_Rank(Ts_Return(close, 1)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_42(
