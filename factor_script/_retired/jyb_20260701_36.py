...
def factor_jyb_20260701_36(start_date, end_date, md_data_file, return_fillna_dic=False):
    """收益率与20日收益率标准差之比的0.5次方带符号幂的截面排名

    复现公式: Cs_Rank(SignedPower(Div(Ts_Return(close, 1), Ts_Std(Ts_Return(close, 1), 20)), 0.5))
    原始分数: 32.44
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_36' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(SignedPower(Div(Ts_Return(close, 1), Ts_Std(Ts_Return(close, 1), 20)), 0.5))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_36(
