...
# ---- 因子元信息 ----
FACTOR_NAME = 'Turnover_Volatility_Inverse_Product'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Turnover_Volatility_Inverse_Product(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去20日换手率标准差的倒数乘以过去20日换手率均值的倒数的截面排名

    复现公式: Cs_Rank(Mul(Inverse(Ts_Std(turn, 20)), Inverse(Ts_Mean(turn, 20))))
    复现分数: 58.78
    """
    ...
    # Mul(Inverse(std_20), Inverse(mean_20))
    result = (1.0 / std_20) * (1.0 / mean_20)
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_Turnover_Volatility_Inverse_Product(
