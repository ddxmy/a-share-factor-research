...
# ---- 因子元信息 ----
FACTOR_NAME = 'Turnover_Stability_Neg_Power_Rank'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Turnover_Stability_Neg_Power_Rank(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去20日换手率标准差的倒数乘以过去20日换手率均值的负0.5次方的截面排名

    复现公式: Cs_Rank(Mul(Inverse(Ts_Std(turn, 20)), Power(Ts_Mean(turn, 20), -0.5)))
    复现分数: 55.50
    """
    ...
    # Mul(Inverse(std_20), Power(mean_20, -0.5)) = (1/std) * (mean^-0.5)
...
if __name__ == '__main__':
    ...
    factor_df = factor_Turnover_Stability_Neg_Power_Rank(
