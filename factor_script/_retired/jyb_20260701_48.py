...
def factor_jyb_20260701_48(start_date, end_date, md_data_file, return_fillna_dic=False):
    """10日指数衰减加权的20日收盘价/VWAP对换手率残差的绝对值

    复现公式: Ts_DecayExp(Abs(Ts_Residual(Div(close, vwap), turn, 20)), 10)
    原始分数: 44.06
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_48' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Ts_DecayExp(Abs(Ts_Residual(Div(close, vwap), turn, 20)), 10)")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_48(
