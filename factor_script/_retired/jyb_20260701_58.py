...
def factor_jyb_20260701_58(start_date, end_date, md_data_file, return_fillna_dic=False):
    """20日涨跌幅平均绝对偏差与20日换手率中位数之比的截面排名

    复现公式: Cs_Rank(Div(Ts_MAD(Ts_Return(close, 1), 20), Ts_Median(turn, 20)))
    原始分数: 48.67
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_58' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Div(Ts_MAD(Ts_Return(close, 1), 20), Ts_Median(turn, 20)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_58(
