...
# ---- 因子元信息 ----
FACTOR_NAME = 'Turnover_Stability_Inv_Power_Rank'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Turnover_Stability_Inv_Power_Rank(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去20日换手率标准差的倒数乘以过去20日换手率均值的倒数的0.5次方的截面排名

    复现公式: Cs_Rank(Mul(Inverse(Ts_Std(turn, 20)), Power(Inverse(Ts_Mean(turn, 20)), 0.5)))
    复现分数: 46.78
    """
    ...
    mean_20 = turn_panel.rolling(20, min_periods=1).mean()
    # Mul(Inverse(std_20), Power(Inverse(mean_20), 0.5)) = (1/std) * ((1/mean)^0.5)
    result = (1.0 / std_20) * ((1.0 / mean_20) ** 0.5)
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_Turnover_Stability_Inv_Power_Rank(
