...
# ---- 因子元信息 ----
FACTOR_NAME = 'Cumulative_VWAP_Deviation_5d'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Cumulative_VWAP_Deviation_5d(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去5日VWAP偏离百分比的累加和，在全市场中的排名（保留方向）

    复现公式: Cs_Rank(Ts_Sum(Div(Sub(close, vwap), vwap), 5))
    原始分数: 32.8 | 复现分数: 32.78
    """
    ...
    # 5日累加和（保留方向）
    sum_5 = vwap_dev_pct.unstack().rolling(5, min_periods=1).sum()
    # Cs_Rank
    factor_df = pd.DataFrame()
    factor_df[factor_name] = sum_5.rank(axis=1, pct=True).stack()
...
if __name__ == '__main__':
    ...
    factor_df = factor_Cumulative_VWAP_Deviation_5d(
