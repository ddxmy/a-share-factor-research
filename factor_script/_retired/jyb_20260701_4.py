...
def factor_jyb_20260701_4(start_date, end_date, md_data_file, return_fillna_dic=False):
    """15日线性衰减加权的日振幅率（(high-low)/close）的截面排名

    复现公式: Cs_Rank(Ts_DecayLinear(Div(Sub(high, low), close), 15))
    原始分数: 45.44
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_4' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Ts_DecayLinear(Div(Sub(high, low), close), 15))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_4(
