...
def factor_jyb_20260701_5(start_date, end_date, md_data_file, return_fillna_dic=False):
    """15日WMA加权的日振幅率除以换手率的截面排名

    复现公式: Cs_Rank(Div(Ts_WMA(Div(Sub(high, low), close), 15), turn))
    原始分数: 33.61
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_5' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Div(Ts_WMA(Div(Sub(high, low), close), 15), turn))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_5(
