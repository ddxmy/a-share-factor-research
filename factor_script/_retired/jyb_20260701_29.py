...
def factor_jyb_20260701_29(start_date, end_date, md_data_file, return_fillna_dic=False):
    """14日RSI与20日收益率标准差之比的截面排名

    复现公式: Cs_Rank(Div(RSI(close, 14), Ts_Std(Ts_Return(close, 1), 20)))
    原始分数: 58.83
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_29' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Div(RSI(close, 14), Ts_Std(Ts_Return(close, 1), 20)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_29(
