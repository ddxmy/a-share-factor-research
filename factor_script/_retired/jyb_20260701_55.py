...
def factor_jyb_20260701_55(start_date, end_date, md_data_file, return_fillna_dic=False):
    """（收盘价/20日最高价90%分位数）×（1/20日收益率标准差）的截面排名

    复现公式: Cs_Rank(Mul(Div(close, Ts_Quantile(high, 20, 0.9)), Inverse(Ts_Std(Ts_Return(close, 1), 20))))
    原始分数: 60.17
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_55' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Mul(Div(close, Ts_Quantile(high, 20, 0.9)), Inverse(Ts_Std(Ts_Return(close, 1), 20))))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_55(
