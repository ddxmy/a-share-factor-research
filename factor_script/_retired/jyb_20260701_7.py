...
def factor_jyb_20260701_7(start_date, end_date, md_data_file, return_fillna_dic=False):
    """10日指数衰减加权的20日收益率趋势强度的截面ZScore

    复现公式: Cs_ZScore(Ts_DecayExp(Trend_Strength(Ts_Return(close, 1), 20), 10))
    原始分数: 47.56
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_7' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_ZScore(Ts_DecayExp(Trend_Strength(Ts_Return(close, 1), 20), 10))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_7(
