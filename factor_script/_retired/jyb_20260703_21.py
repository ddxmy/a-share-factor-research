...
def factor_jyb_20260703_21(start_date, end_date, md_data_file, return_fillna_dic=False):
    """高波动时取收益/波动倒数截面排名，否则取反向换手排名

    复现公式: If(Greater(Volatility_Ratio(5, 20), 1.5), Cs_Rank(Div(Ts_Return(close, 1), Inverse(Ts_Std(Ts_Return(close, 1), 20)))), Neg(Cs_Rank(turn)))
    原始分数: 52.8
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_21' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("If(Greater(Volatility_Ratio(5, 20), 1.5), Cs_Rank(Div(Ts_Return(close, 1), Inverse(Ts_Std(Ts_Return(close, 1), 20)))), Neg(Cs_Rank(turn)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_21(
