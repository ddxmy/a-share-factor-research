...
def factor_jyb_20260701_50(start_date, end_date, md_data_file, return_fillna_dic=False):
    """10日指数衰减加权的15日换手率趋势强度0.5次方带符号幂的截面排名

    复现公式: Cs_Rank(Ts_DecayExp(SignedPower(Trend_Strength(turn, 15), 0.5), 10))
    原始分数: 35.83
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_50' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Ts_DecayExp(SignedPower(Trend_Strength(turn, 15), 0.5), 10))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_50(
