...
# ---- 因子元信息 ----
FACTOR_NAME = 'Turnover_Stability_Rank_60d_Mean'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Turnover_Stability_Rank_60d_Mean(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去20日换手率标准差的倒数除以过去60日换手率均值的截面排名

    复现公式: Cs_Rank(Div(Inverse(Ts_Std(turn, 20)), Ts_Mean(turn, 60)))
    复现分数: 53.33
    """
    ...
    start_date_ = int(get_tradingday(None, start_date=20120101,
                      end_date=start_date, shift=-70)[0])
    ...
    # Ts_Mean(turn, 60): 过去60日换手率均值
    mean_60 = md_data['turn'].unstack().rolling(60, min_periods=1).mean()
    # Div(Inverse(std_20), mean_60): 标准差倒数除以均值
    result = (1.0 / std_20) / mean_60
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_Turnover_Stability_Rank_60d_Mean(
