...
def factor_jyb_20260701_46(start_date, end_date, md_data_file, return_fillna_dic=False):
    """10日MAD加权的20日收益率对换手率残差的截面排名

    复现公式: Cs_Rank(Mul(Ts_MAD(turn, 10), Ts_Residual(Ts_Return(close, 1), turn, 20)))
    原始分数: 53.06
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_46' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Mul(Ts_MAD(turn, 10), Ts_Residual(Ts_Return(close, 1), turn, 20)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_46(
