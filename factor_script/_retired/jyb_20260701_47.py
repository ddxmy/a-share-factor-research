...
def factor_jyb_20260701_47(start_date, end_date, md_data_file, return_fillna_dic=False):
    """15日(收盘价-VWAP)/VWAP方差平方根的截面排名

    复现公式: Cs_Rank(Sqrt(Ts_Var(Div(Sub(close, vwap), vwap), 15)))
    原始分数: 45.11
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_47' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(Sqrt(Ts_Var(Div(Sub(close, vwap), vwap), 15)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_47(
