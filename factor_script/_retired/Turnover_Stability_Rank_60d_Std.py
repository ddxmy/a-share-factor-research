...
# ---- 因子元信息 ----
FACTOR_NAME = 'Turnover_Stability_Rank_60d_Std'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Turnover_Stability_Rank_60d_Std(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去60日换手率标准差的倒数除以过去20日换手率均值的截面排名

    复现公式: Cs_Rank(Div(Inverse(Ts_Std(turn, 60)), Ts_Mean(turn, 20)))
    复现分数: 55.22
    """
    ...
    # Ts_Std(turn, 60): 过去60日换手率标准差
    std_60 = md_data['turn'].unstack().rolling(60, min_periods=1).std()
    # Ts_Mean(turn, 20): 过去20日换手率均值
    mean_20 = md_data['turn'].unstack().rolling(20, min_periods=1).mean()
    # Div(Inverse(std_60), mean_20)
    result = (1.0 / std_60) / mean_20
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_Turnover_Stability_Rank_60d_Std(
