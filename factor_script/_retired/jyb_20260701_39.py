...
def factor_jyb_20260701_39(start_date, end_date, md_data_file, return_fillna_dic=False):
    """收益率截面排名 × exp(-10日换手率标准差)

    复现公式: Mul(Cs_Rank(Ts_Return(close, 1)), Exp(Neg(Ts_Std(turn, 10))))
    原始分数: 49.83
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_39' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Mul(Cs_Rank(Ts_Return(close, 1)), Exp(Neg(Ts_Std(turn, 10))))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_39(
