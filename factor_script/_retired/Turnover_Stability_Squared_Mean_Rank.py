...
# ---- 因子元信息 ----
FACTOR_NAME = 'Turnover_Stability_Squared_Mean_Rank'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Turnover_Stability_Squared_Mean_Rank(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去20日换手率标准差的倒数除以过去20日换手率均值的平方的截面排名

    复现公式: Cs_Rank(Div(Inverse(Ts_Std(turn, 20)), Power(Ts_Mean(turn, 20), 2)))
    复现分数: 62.33
    """
    ...
    # Div(Inverse(std_20), Power(mean_20, 2)) = (1/std) / (mean^2)
    result = (1.0 / std_20) / (mean_20 ** 2)
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_Turnover_Stability_Squared_Mean_Rank(
