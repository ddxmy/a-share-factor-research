...
def factor_jyb_20260703_6(start_date, end_date, md_data_file, return_fillna_dic=False):
    """收益分布厚尾时取换手率10日均值排名，否则取换手率20日标准差排名

    复现公式: If(Greater(Ts_Kurtosis(Ts_Return(close, 1), 20), 3), Cs_Rank(Ts_Mean(turn, 10)), Cs_Rank(Ts_Std(turn, 20)))
    原始分数: 60.2
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_6' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("If(Greater(Ts_Kurtosis(Ts_Return(close, 1), 20), 3), Cs_Rank(Ts_Mean(turn, 10)), Cs_Rank(Ts_Std(turn, 20)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_6(
