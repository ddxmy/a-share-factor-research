...
def factor_jyb_20260701_57(start_date, end_date, md_data_file, return_fillna_dic=False):
    """15日指数衰减加权的|VWAP-开盘价|/收盘价绝对值平方根的截面百分位

    复现公式: Cs_Percentile(Ts_DecayExp(Sqrt(Abs(Div(Sub(vwap, open), close))), 15))
    原始分数: 39.39
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_57' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Percentile(Ts_DecayExp(Sqrt(Abs(Div(Sub(vwap, open), close))), 15))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_57(
