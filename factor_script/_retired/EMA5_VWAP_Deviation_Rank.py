...
# ---- 因子元信息 ----
FACTOR_NAME = 'EMA5_VWAP_Deviation_Rank'
...
def factor_EMA5_VWAP_Deviation_Rank(start_date, end_date, md_data_file, return_fillna_dic=False):
    """过去5日VWAP偏离百分比的指数平滑值，在全市场中的排名

    复现公式: Cs_Rank(Ts_EMA(Div(Sub(close, vwap), vwap), 5))
    原始分数: 52 | 复现分数: 52.00
    ...
    Returns:
        pd.DataFrame: 包含因子列 'EMA5_VWAP_Deviation_Rank' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    start_date_ = int(get_tradingday(None, start_date=20120101,
                      end_date=start_date, shift=-15)[0])
    ...
    # Ts_EMA(..., 5): 5日指数平滑
    ema_5 = vwap_dev_pct.unstack().ewm(span=5).mean()

    # Cs_Rank(...): 截面排名
    factor_df = pd.DataFrame()
    factor_df[factor_name] = ema_5.rank(axis=1, pct=True).stack()
...
if __name__ == '__main__':
    ...
    factor_df = factor_EMA5_VWAP_Deviation_Rank(
