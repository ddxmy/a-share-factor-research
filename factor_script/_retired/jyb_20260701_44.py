...
def factor_jyb_20260701_44(start_date, end_date, md_data_file, return_fillna_dic=False):
    """（开盘价相对VWAP偏离率的1.5次方带符号幂 + 换手率平方根）的截面ZScore

    复现公式: Cs_ZScore(Add(SignedPower(Div(Sub(open, vwap), vwap), 1.5), Sqrt(turn)))
    原始分数: 45.94
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260701_44' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_ZScore(Add(SignedPower(Div(Sub(open, vwap), vwap), 1.5), Sqrt(turn)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260701_44(
