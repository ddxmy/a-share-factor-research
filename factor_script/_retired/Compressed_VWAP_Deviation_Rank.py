...
# ---- 因子元信息 ----
FACTOR_NAME = 'Compressed_VWAP_Deviation_Rank'
FACTOR_TYPE = 'T-1_factor'

_fill_cache: dict = {}


def factor_Compressed_VWAP_Deviation_Rank(start_date, end_date, md_data_file, return_fillna_dic=False):
    """对VWAP偏离做"压缩式"变换：x / (1+|x|)

    复现公式: Cs_Rank(Div(Div(Sub(close,vwap),vwap), Add(1, Abs(Div(Sub(close,vwap),vwap)))))
    ...
    # 压缩式变换: x / (1 + |x|)
    compressed = vwap_dev_pct / (1 + np.abs(vwap_dev_pct))
    # Cs_Rank
...
if __name__ == '__main__':
    ...
    factor_df = factor_Compressed_VWAP_Deviation_Rank(
