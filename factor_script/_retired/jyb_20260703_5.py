...
def factor_jyb_20260703_5(start_date, end_date, md_data_file, return_fillna_dic=False):
    """正偏且低波动时取收盘/均价，否则取换手率倒数的截面排名

    复现公式: Cs_Rank(If(And(Greater(Ts_Skewness(Ts_Return(close, 1), 20), 0), Less(Volatility_Ratio(5, 20), 1)), Div(close, vwap), Inverse(turn)))
    原始分数: 33.3
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_5' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    result_panel = executor.compute("Cs_Rank(If(And(Greater(Ts_Skewness(Ts_Return(close, 1), 20), 0), Less(Volatility_Ratio(5, 20), 1)), Div(close, vwap), Inverse(turn)))")
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_5(
