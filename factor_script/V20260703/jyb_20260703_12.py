...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260703_12'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_ZScore(Exp(Power(Sign(Div(close, vwap)), 2)))'

def factor_jyb_20260703_12(start_date, end_date, md_data_file, return_fillna_dic=False):
    """exp(收盘/均价的符号平方)的截面Z-Score（均价偏离指数放大）
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_12' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # sign(close/vwap)^2 → exp
    vwap_sign = np.sign(close_panel / (vwap_panel + 1e-12))
    exp_val = np.exp(vwap_sign ** 2)

    # Cs_ZScore(...): 截面Z-Score
    cs_mean = exp_val.mean(axis=1)
    cs_std = exp_val.std(axis=1)
    zscore = (exp_val - cs_mean) / (cs_std + 1e-12)
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_12(
