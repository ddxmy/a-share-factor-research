...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260703_7'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_Median(Div(close, Ts_Median(close, 20)))'

def factor_jyb_20260703_7(start_date, end_date, md_data_file, return_fillna_dic=False):
    """收盘价/20日中位数比值的截面中位数（价格中位偏离）
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_7' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # close / 20日中位数
    close_median_20 = close_panel.rolling(20, min_periods=1).median()
    price_ratio = close_panel / (close_median_20 + 1e-12)

    # Cs_Median(...): 截面中位数（广播到每只股票）
    cs_median_df = price_ratio.copy()
    cs_median_df[:] = price_ratio.median(axis=1).values[:, None]
    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = cs_median_df.stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_7(
