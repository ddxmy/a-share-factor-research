...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260703_35'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_Percentile(Div(Ts_Median(Div(Add(high, low), 2), 10), Sub(Ts_Max(high, 20), Ts_Min(low, 20))))'

def factor_jyb_20260703_35(start_date, end_date, md_data_file, return_fillna_dic=False):
    """10日中位价/20日振幅比值的截面百分位（价格位置/振幅）
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_35' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # 10日中位价 / 20日振幅
    median_price = ((high_panel + low_panel) / 2).rolling(10, min_periods=1).median()
    price_range = high_panel.rolling(20, min_periods=1).max() - low_panel.rolling(20, min_periods=1).min()
    ratio = median_price / (price_range + 1e-12)

    # Cs_Percentile(...): 截面百分位
    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = ratio.rank(axis=1, pct=True).stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_35(
