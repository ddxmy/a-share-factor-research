...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260703_46'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_Rank(Power(Sign(Ts_Median(Div(high, close), 20)), 2))'

def factor_jyb_20260703_46(start_date, end_date, md_data_file, return_fillna_dic=False):
    """高价/收盘价的20日中位数的符号平方的截面排名（上影线程度）
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_46' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # high/close 的20日中位数 → 符号平方
    high_close_ratio = high_panel / (close_panel + 1e-12)
    median_ratio = high_close_ratio.rolling(20, min_periods=1).median()
    signed_sq = np.sign(median_ratio) ** 2

    # Cs_Rank(...): 截面排名
    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = signed_sq.rank(axis=1, pct=True).stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_46(
