...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260703_8'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_Rank(Clip(Div(close, Ts_Max(high, 5)), 0, 2))'

def factor_jyb_20260703_8(start_date, end_date, md_data_file, return_fillna_dic=False):
    """收盘价/5日最高价比值截断至[0,2]的截面排名（近期高点接近度）
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_8' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # close / 5日最高价, 截断至[0, 2]
    high_max_5 = high_panel.rolling(5, min_periods=1).max()
    price_ratio = (close_panel / (high_max_5 + 1e-12)).clip(lower=0, upper=2)

    # Cs_Rank(...): 截面排名
    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = price_ratio.rank(axis=1, pct=True).stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_8(
