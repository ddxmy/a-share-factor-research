...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260703_20'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_ZScore(Ts_DecayExp(Ts_Residual(Ts_Return(close, 1), Log(amt), 20), 10))'

def factor_jyb_20260703_20(start_date, end_date, md_data_file, return_fillna_dic=False):
    """收益对成交额对数20日残差的10日衰减加权截面Z-Score
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_20' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # 1日收益率 ~ ln(amt) 的20日滚动回归残差
    log_amt = np.log(amt_panel + 1e-12)
    x_mean = log_amt.rolling(20, min_periods=1).mean()
    y_mean = ret_1d.rolling(20, min_periods=1).mean()
    x_var = ((log_amt - x_mean) ** 2).rolling(20, min_periods=1).mean()
    xy_cov = ((log_amt - x_mean) * (ret_1d - y_mean)).rolling(20, min_periods=1).mean()
    beta = xy_cov / (x_var + 1e-12)
    residual = ret_1d - (y_mean + beta * (log_amt - x_mean))

    # 残差 → 10日衰减加权
    decayed = residual.ewm(alpha=0.1, min_periods=1).mean()

    # Cs_ZScore(...): 截面Z-Score
    cs_mean = decayed.mean(axis=1)
    cs_std = decayed.std(axis=1)
    zscore = (decayed - cs_mean) / (cs_std + 1e-12)
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_20(
