...
# ---- 因子元信息 ----
FACTOR_NAME = 'jyb_20260703_10'
FACTOR_TYPE = 'T-1_factor'
FORMULA = 'Cs_Winsorize(Mul(Ts_EMA(Sqrt(Volatility_Ratio(5, 20)), 10), turn), 0.05, 0.95)'

def factor_jyb_20260703_10(start_date, end_date, md_data_file, return_fillna_dic=False):
    """波动率比平方根的10日EMA×换手率的5%~95%缩尾
    ...
    Returns:
        pd.DataFrame: 包含因子列 'jyb_20260703_10' 的 DataFrame，index 为 (dt, Ticker)
    """
    ...
    # 5/20波动率比率 → 平方根 → 10日EMA
    vol_ratio = close_panel.rolling(5, min_periods=1).std() / (close_panel.rolling(20, min_periods=1).std() + 1e-12)
    vol_sqrt_ema = np.sqrt(vol_ratio).ewm(span=10).mean()

    # × 换手率
    factor_val = vol_sqrt_ema * turn_panel

    # Cs_Winsorize(..., 0.05, 0.95): 截面缩尾（按截面5%/95%分位数clip）
    lo = factor_val.quantile(0.05, axis=1)
    hi = factor_val.quantile(0.95, axis=1)
    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = factor_val.clip(lower=lo, upper=hi, axis=0).stack()
    return factor_df
...
if __name__ == '__main__':
    ...
    factor_df = factor_jyb_20260703_10(
