"""Generated factor script for admitted factor #3.

Formula:
    Neg(Ts_ZScore(Div(Sub(close, vwap), vwap), 20))
"""
import os
import sys
...
FACTOR_NAME = 'xzt_20260707_2'
FACTOR_TYPE = "T-1_factor"
FORMULA = 'Neg(Ts_ZScore(Div(Sub(close, vwap), vwap), 20))'
...
def factor_xzt_20260707_2(start_date, end_date, md_data_file, return_fillna_dic=False):
    ...
    # ---- 因子计算 ----
    # VWAP偏离百分比 → 20日ZScore → 取负
    vwap_dev_pct = (md_data['close'] - md_data['vwap']) / md_data['vwap']
    panel = vwap_dev_pct.unstack()

    # 20日ZScore
    zscore_mean = panel.rolling(window=20, min_periods=1).mean()
    zscore_std = panel.rolling(window=20, min_periods=1).std()
    zscore = (panel - zscore_mean) / zscore_std
...
if __name__ == "__main__":
    ...
    factor_df = factor_xzt_20260707_2(start_date, end_date, md_data_file)
