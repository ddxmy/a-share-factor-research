"""Generated factor script for admitted factor #4.

Formula:
    Ts_EMA(Div(Sub(high, low), close), 20)
"""
import os
import sys
...
FACTOR_NAME = 'xzt_20260707_3'
FACTOR_TYPE = "T-1_factor"
FORMULA = 'Ts_EMA(Div(Sub(high, low), close), 20)'
...
def factor_xzt_20260707_3(start_date, end_date, md_data_file, return_fillna_dic=False):
    ...
    # ---- 因子计算 ----
    # 日内振幅(close范围内) / close → 20日EMA
    daily_range = (md_data['high'] - md_data['low']) / md_data['close']
    ema_val = daily_range.unstack().ewm(span=20, adjust=False).mean()

    factor_df = pd.DataFrame()
    factor_df[FACTOR_NAME] = ema_val.stack()
...
if __name__ == "__main__":
    ...
    factor_df = factor_xzt_20260707_3(start_date, end_date, md_data_file)
